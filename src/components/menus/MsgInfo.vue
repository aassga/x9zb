<template>
  ...
  <div class="HuyaXWebhMessageuserlist">
    <div class="HuyaXWebhMessageuserlist_header">
    ...
    <div class="user_info_content">
      <div class="user_info_header">
        <el-avatar
          ...
          class="user_info_header_avater"
        >{{userInfo.user_nickname ? userInfo.user_nickname.substr(0,1) : ''}}</el-avatar>
        <div class="user_info_header_name">{{userInfo.user_nickname}}</div>
        <div class="user_info_header_nick_area">
          <template v-if="!isEdited">
            <div
              class="user_info_header_nick"
              :title="userInfo.user_nickname"
            >{{userInfo.user_nickname}}</div>
      ...
      <div class="user_info_actions">
        ...
        <div class="el-icon-delete user_info_actions_del" @click="del()"></div>
      </div>
      <div class="user_info_details">
        <div class="user_info_title">个人资讯</div>
        <div
          class="info_list"
        >注册时间： {{ userInfo.create_time ? checkTime(userInfo.create_time) : "-" }}</div>
        <div class="info_list">终端类型： {{ userInfo.device_type ? userInfo.device_type : "-" }}</div>
        <div class="info_list">观赛次数： {{ userInfo.view_times ? userInfo.view_times : "-" }}</div>
        <div class="info_list">最后观看： {{ userInfo.last_view_room ? userInfo.last_view_room : "-" }}</div>
        <div class="info_list">设置纪录： -</div>
      </div>
...
export default {
  ...
  watch: {
    ...
    user_id (newV, oldV) {
      console.log('user_id', newV)
      if (newV) {
        this.getUserInfo(newV)
        this.getUserTag(newV)
      }
    },
    show (newV, oldV) {
  ...
  methods: {
    ...
    async getChatMemberList () {
      ...
      const res = await this.$store.dispatch('getChatRoomUserList', { ...data })
      ...
      if (res.code == 0) {
        ...
        if (res.data[0].user_id == userInfo.user_id) {
          this.user_id = res.data[1].user_id
        } else {
    ...
    onHandleChangeNickName () {
      const editNickName = this.editNickName
      if (editNickName == this.userInfo.user_nickname) {
        return
      }
    ...
    async del () {
      const _this = this
      this.$confirm('确定要删除此私聊对话吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消'
      }).then(async () => {
        const item = _this.userInfo
        const user_id = _this.user_id
        const res = await _this.$store.dispatch('delGroup', {
        ...
        if (res.code == 0) {
          ...
          _this.$emit('onHandleDelChat')
        }
      })
    },
    // 加群
    addGroup () {
      this.$emit('onHandleAddToGroup', 0, this.userInfo)
    },
    // 禁言
    mute () {
      const item = this.userInfo
      ...
      this.$store
        .dispatch('mute', {
          ...
          mute: this.userInfo.is_mute == 0 ? 1 : 2, // 1:禁言, 2:解禁
    ...
    freeze () {
      ...
      this.$store
        .dispatch('freeze', {
          type: this.roomDetailData.type || 0,
          freeze: this.userInfo.is_freeze == 0 ? 1 : 2, // 1:凍結, 2:解凍
          token: this.imUserInfo.token,
          vid: this.roomDetailData.vid,
          fd: this.roomDetailData.fd,
          user_id: user_id
