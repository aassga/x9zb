<template lang="pug">
  div
    ...
    el-table.mt-2(
      ...
      el-table-column(label="档案类别" prop="type")
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      ...
      disk (state) {
        return state[this.service].disk
      }
    })
  },
  created () {
    ...
    async connect ({ name }) {
      try {
        ...
        socketChunks.forEach((chunk, index) => {
          ...
          const ws = new WebSocket(this.disk.upload_url[0])
    ...
    socketInit (list, socket) {
      ...
      return new Promise((resolve, reject) => {
        ...
        ws.addEventListener('close', async () => {
          ...
          socket.ws = new WebSocket(this.disk.upload_url[0])
          await this.socketInit(list, socket)
