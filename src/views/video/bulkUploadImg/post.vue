<template lang="pug">
  .postbg.h-100(
    ...
    .flex-center.px-3
      ...
      .money_area(v-if="user_type == 2")
        i.ic_money
        input(type="number" v-model="cose" @blur="changeCose")
      //- .position_area
      //-   i.ic_position
      //-   .position_name 東京都
      //- .text_area.content
      //-   textarea(placeholder="内容" type="textarea" :disableClear="true" v-model= "requestData.content" @blur="checkPostInfo")
      ContentBox(@changeContent="changeContent")
</template>
...
import ContentBox from '@/components/bulkUploadImg/content.vue'
...
export default {
  ...
  data () {
    return {
      ...
      requestData: {
        title: '',
        content: '',
        option1: { val: 181 }, // 选择版块
        option2: { val: 6736 }, // 选择主题 免费贴文6563 套餐贴文6566 单价贴文6569
        photo: [],
  ...
  computed: {
    ...
    ...mapState({
      ...
      imgfiles: ({ post }) => post.imgfiles,
      currentCity: ({ user }) => user.currentCity// 当前城市id
    }),
    ...mapGetters([
      'user_type'// 用户类型
    ]),
    // 检测是否要显示vip
    checkShowVip () {
      const { user_type, model_combo, tipsType, postType } = this
      if (user_type == 2 && model_combo.length > 0 && postType == 1) {
        return true
      } else {
        return false
      }
  ...
  watch: {
    ...
    checkCanPostData (newData, oldData) {
      this.$emit('CheckStatus', newData)
    }
  },
  mounted () {
    this.requestData.postType = this.postType
    console.log(this.imgfiles)
    this.requestData.photo = this.imgfiles
  },
  async created () {
  ...
  methods: {
    // 修改内容
    changeContent (htmlContent, textContent) {
      console.log(htmlContent, textContent)
      this.requestData.content = htmlContent
      this.requestData.htmlContent = htmlContent
    },
    // getfid
    getfid (typeid, cose, level) {
      if (typeid == 1) { // 普通贴文
        if (cose && parseFloat(cose) > 0) {
          return 790
        }
        if (level) {
          return 790
    ...
    // getTypeid
    getTypeid (typeid, cose, level) {
      if (typeid == 1) { // 普文
        if (cose && parseFloat(cose) > 0) { // 付费
          return 6740
        }
        if (level) { // 套餐
          return 6738
    ...
    // 输入cose
    changeCose () {
      if (this.cose != null && this.cose != '' && parseFloat(this.cose) > 0) {
        this.level = null
      }
    },
    // 提交按钮内容校验
    checkPostInfo () {
      ...
      console.log(this)
      this.$emit('CheckStatus', isAlready)
      // return isAlready
    },
    ...
    // 图片上传情况  watch处使用较多
    async sendAPI () {
      const { requestData, requestData: { photo }, uploadedInfo, imgFailIndex, level, cose, currentCity } = this
      if (photo.length == (uploadedInfo.length + imgFailIndex.length)) {
        console.log('photo的长度')
        const postData = {
          fid: this.getfid(requestData.postType, cose, level), // 默认158,
          typeid: this.getTypeid(requestData.postType, cose, level), //
          subject: requestData.content.length > 75 ? requestData.content.substring(1, 75) : requestData.content,
          message: requestData.content,
          images: uploadedInfo,
          level: level || 0,
          postType: requestData.postType, // 1为普通 2为限时
          cose: cose ? parseFloat(cose) : 0,
          latitude: currentCity.latitude ? currentCity.latitude : requestData.latitude,
          longitude: currentCity.longitude ? currentCity.longitude : requestData.longitude,
          cities_id: currentCity.id ? currentCity.id : requestData.cities_id,
        ...
        if (this.getUploadImgFailCount() > 0) {
            ...
            MessageBox({
              ...
              if (action === 'cancel') {
                ...
                if (isSuc) {
                  ...
                  this.$store.commit('SET_PROGRESSBAR', false)
                }
              } else {
          ...
          if (isSuc.status == 200) {
            ...
            // })
            // this.dataInit()
            this.$emit('getTid', isSuc.data.tid)
    ...
    async sendApiWithImg (postData) {
      if (postData.images.length > 0) {
        return await this.$store.dispatch('upLoadVideoInfo', postData)
      } else {
