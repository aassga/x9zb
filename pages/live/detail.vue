...
<template>
	<view class="detail detail_box">
		...
		<view class="video" @click="play(1)" v-if="!hidevideo" :style="{paddingTop:show1?'108rpx':'0','height':show1?'528rpx':'420rpx'}">
		...
		<view :class="['tabs_context',show1&&!hidevideo ? '' : 'tabs_context_max']">
			<newDetail1 :headShow="show1&&!hidevideo" :activeIndex="activeIndex2" @onHandleClickItem="onHandleClickItem" :current="current"
				v-if="current==1&&showTabs.includes(current)&&!showMsgInfo" :list="messageList"></newDetail1>
			<MessageInfo :headShow="show1&&!hidevideo" @close="onHandleMsgInfoBack" v-if="current==1 && showMsgInfo" :current="current"
				:roomInfo="roomInfo">
			</MessageInfo>
			<detail1 :headShow="show1&&!hidevideo" :giftList="giftList" :current="current" :showMsgInfo="showMsgInfo" @getShow="getShow" @leaveRoom="leaveRoom"
			    :qsVid="qsVid"
				v-if="qsVid&&showTabs.includes(current)||current==1 && showMsgInfo" :roomInfo="roomInfo" :roomDetailData="roomInfo" :userInfo="userInfo" @getMessageList="getMessageList"></detail1>
	...
	export default {
		...
		methods: {
			...
			copyTextLink() {
				...
				qrUrlContent.value = window.location.origin +"/room/"+ getQueryString().id+"?&c="+getQueryString().channel_code;
				qrUrlContent.select()
