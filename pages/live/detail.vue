...
<template>
	<view class="detail detail_box">
		...
		<image v-if="!hidevideo" class="red-envelope-icon" src="/static/images/daily/red-envelope.png" mode="" @click="showRedEnvelopeDialog = true" :style="{top: show1 ? '648rpx' : '540rpx'}"></image>
		<view class="video" @click="play(1)" v-if="!hidevideo"
			:style="{paddingTop:show1?'108rpx':'0','height':show1?'528rpx':'420rpx'}">

			<!-- <image class="play" src="../../static/images/live/play-img.png" mode=""></image> -->
			<view class="counttDownShow" v-if="counttDown" :style="{marginTop:show1?'108rpx':'0'}">
				<view class="center_date">
					<view class="title_date">开赛倒计时：</view>
					<view class="date_out"><span class="d_date">{{counttDownDay[0]}}</span><span
							class="h_date">{{counttDownDay[1]}}</span><span
							class="m_date">{{counttDownDay[2]}}</span><span class="s_date">{{counttDownDay[3]}}</span>
					</view>
					<view class="date_tile"><span class="d_date_d">天</span><span class="h_date_h">小时</span><span
							class="m_date_m">分钟</span><span class="s_date_s">秒</span></view>
					<view class="concern_box_shou">
						<view @click="copyTextLink()" class="txt_control" title="分享"></view>
					</view>
				</view>
			</view>
			<!-- //腾讯 -->
			<!-- <TcVideoPlayer  /> -->
			<!-- 原播放器 -->
			<view class="thumb" ref="video" v-if="videoPlay">

			</view>
			<view class="voice" @click.stop="play(2)" v-if="isVolume && videoPlay">
				取消静音
			</view>
			<image class="thumb" v-if="room.info && !videoPlay" :src="room.info.thumb" mode="aspectFill"></image>
			<view class="video-bar flex" v-if="videoBar && videoPlay" @click.stop="">
				<view class="video-bar-left" @click="play(3)">
					<image :src="isPlay?'/static/images/live/puase.png':'/static/images/live/play.png'" mode=""></image>
					<image src="/static/images/live/reset.png" @click="play(4)" mode=""></image>
				</view>
				<view class="video-bar-right flex-end">
					<text @click="isQuality = !isQuality">{{quality[qualityIndex]}}</text>
					<view class="quality" v-if="isQuality">
						<text v-for="(item,index) in quality" :key="index"
							@click="qualityIndex = index,isQuality = false">{{item}}</text>
					</view>
				</view>
			</view>
			<image class="video_logo" src="/static/images/live/logo_red.png" mode=""></image>
		</view>

		<view @touchmove.stop="(e)=>{}" class="tabs" :class="{'only-chat':onlyChat}">
			<u-tabs :style="{
				width:'620rpx'
			}" :bar-width="50" :list="tabList" font-size="24" active-color="#E3AC72" :is-scroll="true" :current="current"
				@change="change"></u-tabs>
			<view class="tabs-btn flex-center" :class="{'active':userInfo.is_attention == 1||alreadyAttention}">
		...
		<template v-if="chat_msg_close=='0'">
			<view :class="['tabs_context',show1&&!hidevideo ? '' : 'tabs_context_max']">
				<newDetail1 :headShow="show1&&!hidevideo" :activeIndex="activeIndex2"
					@onHandleClickItem="onHandleClickItem" :current="current"
					v-if="current==1&&showTabs.includes(current)&&!showMsgInfo" :list="messageList"></newDetail1>
				<MessageInfo :headShow="show1&&!hidevideo" @close="onHandleMsgInfoBack" v-if="current==1 && showMsgInfo"
					:current="current" :roomInfo="roomInfo">
				</MessageInfo>
				<detail1 :headShow="show1&&!hidevideo" :giftList="giftList" :current="current"
					:showMsgInfo="showMsgInfo" @getShow="getShow" @leaveRoom="leaveRoom" :qsVid="qsVid"
					v-if="qsVid&&showTabs.includes(current)||current==1 && showMsgInfo" :roomInfo="roomInfo"
					:roomDetailData="roomInfo" :userInfo="userInfo" @getMessageList="getMessageList"></detail1>
				<detail2 v-if="current==3" :userInfo="userInfo" @getShow="getShow"></detail2>
				<detail3 v-if="current==4" ref="detail3" :userInfo="userInfo"></detail3>
		...
		<template v-else>
			<view class="tabs_context" :class="{'hasHeader': show1&&!hidevideo}">
				<detail2 v-if="current==0" :userInfo="userInfo" @getShow="getShow"></detail2>
				<detail3 v-if="current==1" ref="detail3" :userInfo="userInfo"></detail3>
		...
		<redEnvelopeDialog v-if="showRedEnvelopeDialog" @close="showRedEnvelopeDialog = false"></redEnvelopeDialog>
		<!-- 下载弹框 -->
		<downLoadModel v-if="showDownLoadModel&&!hidevideo" @close="showDownLoadModel = false" ></downLoadModel>
	</view>
</template>
...
<script>
	...
	export default {
		...
		created() {
			...
			if (this.$store.state.system.chat_msg_close == "1" && getQueryString().hidevideo) {
		...
		methods: {
			...
			play(type) { //视频进度
				if (type == 1) {
					...
					if (this.videoBar) { //如果是开启 5s后自动关闭
						this.i = 0
						this.setInter()
					} else {
				...
				} else if (type == 2) { //解除声音（100%
					this.player.volume(1)
					this.player.muted(false)
					this.isVolume = false
				} else if (type == 3) { //播放/暂停
					if (this.isPlay) {
						this.player.pause()
					} else {
						this.player.play()
			...
			getShow(i) {
				if (!getQueryString().hidevideo) {
					...
					this.$u.post('/api/Member/attention', {
			...
			copyTextLink() {
				...
				qrUrlContent.value = window.location.origin + "/room/" + getQueryString().id + "?&c=" + getQueryString()
					.channel_code;
				qrUrlContent.select()
