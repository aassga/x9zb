	...
	<div class="detail" v-show="showMsgInfo||current!=1" :class="{'app-version':hidevideo,'add-margin':current==1,'is-ios':iosDevice}">
		...
		<div class="gift-box" v-show="isShowGiftBox">
			<div class="gift-header">
				<img class="close" src="/static/images/live/close.png" @click="isShowGiftBox = false">
			</div>
			...
			<div class="balance">
				<div class="left">
					<span>我的钻石: {{is_login() ? parseInt($store.state.info.balances) : 0}}</span>
				</div>
			...
			giftScroll(type) {
				...
				timer = setInterval(() => {
					if (Math.abs(scrollCount) < offsetWidth) {
						this.$refs.giftList.scrollLeft = this.$refs.giftList.scrollLeft + speed;
						scrollCount += speed;
					} else {
						clearInterval(timer);
						timer = null;
