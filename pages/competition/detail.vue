...
<template>
	<view class="detail">
		...
		<view class="detail-video flex" :style="{marginTop:show1?'100rpx':'0'}" v-if="mlive == 1">
			...
			<view class="detail-vs flex">
				...
				<view class="detail-vs-center">
					...
					<view class="detail-vs-num" v-if="base.home_data">{{base.home_data[0]}}-{{base.away_data[0]}}</view>
				</view>
				<view class="detail-vs-left">
			...
			<view class="detail-video-btn flex">
				<text v-if="mlive == 1" @click="mlive = 2">动画直播</text> <!-- <text v-if="base.mlive == 1">|</text> <text>视频直播</text> -->
			</view>
		</view>
		<!--  -->
		<view class="detail-video flex" :style="{marginTop:show1?'100rpx':'0'}" style="height: 244px;" v-else>
			<view class="detail-navBar flex-wrap">
				 <image src="/static/images/competition/right.png" @click="toUrl(mlive)" mode=""></image>
			</view>
			<iframe style="width: 100%;height: 100%;" :src="base.live_animation_url" frameborder="0"></iframe>
		</view>
		<!-- 切换 -->
		<u-tabs :list="tabList" :is-scroll="true" font-size="28" item-width="calc(100%/5)" active-color="#E3AC72" :current="current" @change="change"></u-tabs>
...
<script>
	...
	export default {
		...
		methods:{
			...
			// 获取篮球详情
			getDetail(id){
				this.$u.get('/api/Football/football_match_detail', {id:id}).then(res => {
					// res为服务端返回的数据
...
<style lang="scss">
	.detail-video {
		position: relative;
		min-height: 392rpx;
		background: url(/static/images/competition/video-bg.png)center top / 100% auto no-repeat;
	}
	.detail-navBar {
		position: absolute;
		top: 0;
