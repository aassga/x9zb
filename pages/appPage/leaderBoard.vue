...
<template>
  <view class="detail">
    <view class="numerical-tab flex">
      <view
        class=""
        v-for="(item, index) in numericalList"
        :key="index"
        @click="numericalIndex = index"
    ...
    <view class="detail-list">
      <view class="detail-li flex" v-for="(item, index) in list" :key="index">
        <image
          class="detail-li-class"
          src="/static/images/live/live-9.png"
          mode=""
        ></image>
        ...
        <image
          v-if="item.order == 1"
          class="detail-li-sort"
          src="/static/images/live/live-10.png"
          mode=""
        ></image>
      </view>
    </view>
    <u-empty
      text="暂无豪气"
      v-if="list.length == 0"
      src="/static/images/competition/none.png"
      icon-size="400"
      mode="list"
    ></u-empty>
  </view>
...
export default {
  data() {
    return {
      ...
      numericalIndex: 0,
			userInfo:{
				uid:this.$route.query.anchor_id,
				type:0
			}
  ...
  watch: {
	...
	mounted() {
		this.getList({ uid: this.userInfo.uid, type: this.userInfo.type });
	},
  methods: {
    getList(data) {
      console.log(data);
