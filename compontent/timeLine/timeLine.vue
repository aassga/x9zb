		...
		<view class="time-line">
			...
			<view class="time-line-container" :class="addTypeClass">
				...
				<view class="time-line-list">
					...
					<!-- 时间轴内容块 -->
					<view class="time-line-info" :key="index" :class="{'right-info':item.position == 2}" :id="'timeline'+index" v-for="(item,index) in tlive" v-show="item.position != 0">
						<!-- 内容块内容 -->
						<view class="line-info-content" >
							<!-- 时间轴圆点 -->
							<view class="line-on-round" :style="{ opacity: current[index]&&current[index].is=='ok'?1:0,top:'50px'}" :class="current[index]&&current[index].is=='ok'?comeani:''">
								<image :src="'/static/images/competition/icon'+item.type+'.png'" mode="aspectFill"></image>
							</view>
							<view class="info-content-wrap" :style="{ opacity: current[index]&&current[index].is=='ok'?1:0}" :class="current[index]&&current[index].is=='ok'?comeani:''">
								...
								<view class="info-content">
									<!-- 内容 -->
									<view class="info-txt">{{item.data}}</view>
								</view>
							</view>
		...
		methods:{
			init(){
				try {
					...
					this.HEIGHT= 10000;
					// 添加标志位
					for(let i=0;i<this.tlive.length;i++){
